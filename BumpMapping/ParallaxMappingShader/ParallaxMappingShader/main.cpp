...
GLuint genNormalTexture(const char* filename, GLenum internalformat = GL_RGB, GLenum format = GL_RGB)
{
	AUX_RGBImageRec *ptrImage = NULL;
	GLuint textureID = 0;
	SrImageBmp tmp(IMAGE_READ_ONLY);
	unsigned char* ptr;
	int type;
	if (format == GL_RGB)
		type = IMAGE_RGB;
		...
		type = IMAGE_RGBA;
	tmp.readFile(filename, ptr, type);

	glGenTextures(1, &textureID);
	glBindTexture(GL_TEXTURE_2D, textureID);
	glTexImage2D(GL_TEXTURE_2D, 0, internalformat, tmp.getWidth(), tmp.getHeight(), 0, format, GL_UNSIGNED_BYTE, ptr);
...
void outputMapping()
{
	if (gSwitch == 0)
		...
		printf("Parallax Mapping with Offset Limiting!\n\n");
	else if (gSwitch == 2)
		printf("Steep Parallax Mapping!\n\n");
	else if (gSwitch == 3)
		printf("Relief Parallax Mapping!\n\n");
	else if (gSwitch == 4)
		printf("Parallax Occlusion Mapping Version1!\n\n");
	else
		printf("Parallax Occlusion Mapping Version2!\n\n");
}

void init()
{
	// Create and compile our GLSL program from the shaders
	gProgramId = GenProgram("ParallaxMapping.vert", "ParallaxMapping.frag");
	...
	gShaderSwitch = glGetUniformLocation(gProgramId, "gSwitch");

	// Load the texture
	gDiffTex = genNormalTexture("rock_diff.bmp", GL_RGB, GL_RGB);
	gNormTex = genNormalTexture("rock_norm.bmp", GL_RGBA, GL_RGBA);
...
void renderScene(void)
{
	...
	glRotatef(gRotateAngle, 1.0f, 0.0f, 0.0f);
	...
	glUniform1i(gShaderSwitch, gSwitch);
